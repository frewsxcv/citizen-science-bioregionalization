...
def pivot_taxon_counts_for_clusters(cluster_taxa_stats: ClusterTaxaStatisticsDataFrame) -> pl.DataFrame:
    """
...
class ClusterDistanceMatrix(DataContainer):
    ...
    def build(
        ...
        logger.info(f"Building cluster distance matrix: {X.shape[0]} clusters, {X.shape[1]} taxon IDs")
    ...
    def get_most_similar_clusters(self, cluster_id: int, n: int = 3) -> List[Tuple[int, float]]:
        ...
        cluster_distances = [(cid, float(distances[i])) 

...
import polars as pl
import numpy as np
from src.matrices.cluster_distance import ClusterDistanceMatrix, pivot_taxon_counts_for_clusters, build_X
...
class TestClusterDistanceMatrix(unittest.TestCase):
    def test_pivot_taxon_counts_for_clusters(self):
        ...
        self.assertEqual(result.shape, (2, 3))  # 2 clusters, 3 columns (cluster + 2 taxa)
    ...
    def test_cluster_distance_matrix_build(self):
        ...
        df = pl.DataFrame({
            ...
            "average": [0.05, 0.025, 0.1, 0.015, 0.075, 0.05, 0.04, 0.01, 0.125, 0.035, 0.037, 0.092],
        }, schema={
            "cluster": pl.UInt32(),
            "taxonId": pl.UInt32(),
            "count": pl.UInt32(),
